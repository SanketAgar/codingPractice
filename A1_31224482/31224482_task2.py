...
def cal_stock_revenue(initial_value):
    starting_year = str(read_data()['start_year'])  #converting the input to a string
    current_stock = read_data()['start_stock']  #assigning a more readable variable to the read data for start stock
    total_revenue = read_data()['start_revenue']  #assigning a more readable variable to the read data for start revenue
    ...
    start_year = int(starting_year[:4])  # changing the first 4 chars in the start calender variable to the year
    start_month = int(starting_year[5:6])  # changing the 5th and 6th chars in the start calender variable to the month
    start_date = int(starting_year[7:8])  # changing the 7th and 8th chars in the start calender variable to the day
    NO_YEAR_SIM = 3  #variable for the number of year the simulation will run for after the start date.
    end_year = start_year + NO_YEAR_SIM  #calculating the final year of the simulation
    ...
    total_year_sim = end_year - base_year        #calculates the number of years the simulation has to run for

    for year in range(0, total_year_sim + 1):       #looping through every year the simulation has to run for
        current_year = base_year + year             #updating the current year
        if current_year % 4 == 0 and current_year % 400 != 0 and current_year % 400 == 0:   #checking for leap year and updating values
            day_list[1] = 29
        else:
            day_list[1] = 28
        financial_crisis = current_year - base_year - (2 * crisis_year_count)            #updating financial crisis variable in case of multiple crisis
        if financial_crisis % CRIS_RECUR_FREQUENCY == 0 and financial_crisis_status == 0 and current_year != base_year:    #checking for financial crisis and updating values
            ...
            crisis_year_count = crisis_year_count + 1
        else:
        ...
        for month_number in range(0, 12):                          #looping through the months
            ...
            max_day = day_list[month_number]                        #fetchinf maximum days for the current month
            for day in range(1, max_day + 1):                       #looping through the days
                print(day)

                if current_year == end_year and month_number+1 == start_month and day == start_date:            #checking for end date
                    end_calender = str(end_year)+"%02d"%start_month+"%02d"%start_date                           #creating the end_year variable with the current month and day values
                    if current_stock<0:                                                                         #checking if stock is defaulting
                        current_stock = "The company is selling the product faster than they can restock it, increase start stock and restock value."
                        end_sim = {'end_year': end_calender, 'end_stock': current_stock, 'end_revenue': total_revenue}
                        return end_sim                                                                             #returning the result
                    else :
                        end_sim = {'end_year': end_calender, 'end_stock': (str(math.ceil(current_stock))), 'end_revenue': total_revenue}
                        return end_sim

                else:
                    if current_year > start_year or (current_year==start_year and (month_number+1)>start_month) or (current_year==start_year and (month_number+1) == start_month and day>=start_date): #checking for the start date
                        if day == max_day:                                                      #checking for last day of the month to calculate defective products.
                            defective_month = month_dist * (PER_DEF / 100)
                ...
                if current_stock < 400:                                                     #checking for stock restock criteria
                    current_stock = current_stock + restock_value
...
def write_data(end_sim):          #The write data function needs the dictionary variable end_sim to run
    ...
    data_print.write((str(end_sim['end_year'])) + "\n" + (end_sim['end_stock']) + "\n" + "%0.2f" %
                         end_sim['end_revenue'])  #formatting the output
    data_print.close()


# print(total_revenue)
write_data(cal_stock_revenue(read_data()))   #calling all functions to execute the code.
